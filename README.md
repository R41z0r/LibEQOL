...
- `ShowStandaloneSettingsDialog(frame, options?)` / `HideStandaloneSettingsDialog(frame?)` / `HideSettingsDialog(frame?)` / `IsStandaloneSettingsDialogShown(frame?)` – helpers to open, close, or query the shared settings dialog. `HideStandaloneSettingsDialog` only closes standalone dialogs; `HideSettingsDialog` closes the active dialog in any mode and can be used before unregistering a frame. Existing Edit Mode behavior stays unchanged unless you call them. Omit `options.settings` / `options.buttons` to reuse rows already registered for the frame; pass them directly to build a popup for an unregistered frame instead.
