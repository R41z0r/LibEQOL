...
- `internal:RefreshSettingValues()` – re-runs `Setup` on visible rows so their displayed values match changes you applied in another setting’s `set(...)`.
...
- `RegisterCallback(event, callback)` – `event` is `"enter"`, `"exit"`, `"layout"`, `"layoutadded"`, `"layoutdeleted"`, `"layoutrenamed"`, `"layoutduplicate"`, or `"spec"`.
  ...
  - `layoutadded` receives `(addedLayoutIndex, activateNewLayout, isLayoutImported, layoutType)`; `activateNewLayout=true` means the new layout was activated immediately; `isLayoutImported=true` distinguishes imports from copy/new; `layoutType` is `Enum.EditModeLayoutType`.
