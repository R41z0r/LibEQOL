...
- Call `EditMode.internal:RefreshSettingValues()` if you change other setting values inside a `set(...)` handler and want the visible rows to update immediately.
- `AddFrame` callback signature: `callback(frame, layoutName, point, x, y)`; positions are relative to `UIParent` and `relativePoint` equals `point`.
