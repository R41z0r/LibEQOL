...
Docs:
- Documented the new GitHub Action entrypoint (`R41z0r/LibEQOL@v1.0.0`) for fetching releases in CI, noting it extracts and stages `destination/LibEQOL`.
- Warned against declaring LibEQOL as a BigWigs packager External to avoid leaking into other addons and causing XML/template conflicts; embed it directly instead.
